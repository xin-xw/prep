...
class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        def classifyWord(word):
            sorted_word = sorted(word) #O(KLOGK), k is the length of string
            sorted_word = ''.join(sorted_word) #O(K)
            if sorted_word in anagrams:
                anagrams[sorted_word].append(word)
            else:
                anagrams[sorted_word] = [word]

        for i in range(len(strs)): # O(N), N is # elts in strs
            word = strs[i] 
            classifyWord(word) # O(N * KLOGK)
